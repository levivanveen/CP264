...
Graph* create_graph(int w, int d) {
	...
	//create BLOCK x BLOCK adjecency matrix
...
int add_vertex_graph(Graph *g, Vertex *v) {
	...
	//should not add a duplicate vector
	if (has_vertex_graph(g, v)) {
		printf("Error(add_vertex_graph): Vertex already exists\n");
		return False;
	}
	...
	//assign the new vertex a sequantial number & increment vertex count
...
int has_edge_graph(Graph *g, Edge *e) {
	...
	//Graph does not contain directed of weighted edges
...
int add_edge_graph(Graph *g, Edge *e) {
	...
	//Making sure not to add a duplicate
	if (has_edge_graph(g, e)) {
		printf("Error(add_edge_graph): edge already exists\n");
		return False;
	}

	//check for compatability in terms of weight and directed
	if (e->directed || e->weight) {
		printf("Error(add_edge_graph): incompatible edge\n");
		return False;
	}

	//if necessary, add vertices to the graph
