...
void destroy_edge(Edge **e) {
	assert(e && *e);
	//Destroying an edge should not destroy it's vertices
	(*e)->v1 = NULL;
	(*e)->v2 = NULL;
	free(*e);
	*e = NULL;
...
int is_equal_edge(Edge *e1, Edge *e2) {
	...
	//if one edge is weighted and the other is not, not equal
	if (e1->weight != e2->weight)
		return False;
	//if one edge is dirteceted and the other is not, not equal
	if (e1->directed != e2->directed)
		return False;
	//Reaching here means both edges are equal in terms of wieght and direceted.
	//if vertices match, equal
