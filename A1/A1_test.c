...
void test_get_code() {
	...
	for (i = 0; i < 4; i++) {
		for (j = 0; j < 5; j++)
			printf("get_code(%ld,%c) = %hd\n", serial_number[i], types[j],
					get_code(serial_number[i], types[j]));
		printf("\n");
	}
...
void test_print_description() {
	...
	int i;
	long serial_number[10] = { 210653, 141349, 322459, 237903, 188140, 398225,
			283767, 494660, 556103, 13577 };
...
void test_math_game() {
	...
	int cases[3][8] = { { 10, -1, 30, 4, 20, 2, 40, 3 }, { 10, 2, -30, 2, 18, 5,
			50, 3 }, { 10, 2, -30, 2, 1800, 1, -100, 5 } };
