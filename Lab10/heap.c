...
Data* find_min_heap(Heap *h) {
	...
	int leaves_array[h->size];
	int num_leaves = get_leaves(h, leaves_array);
	Data *minimum = copy_data(&h->array[leaves_array[0]]);

	for (int i = 0; i < num_leaves; i++) {
		if (compare_data(minimum, &h->array[leaves_array[i]]) == 1) {
			minimum = copy_data(&h->array[leaves_array[i]]);
		}
	}
	return minimum;
}
